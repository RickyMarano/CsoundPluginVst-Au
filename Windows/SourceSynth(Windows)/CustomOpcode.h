...
struct MonoMidiState
{
    ...
    {
        // // Rimuovi la nota dallo stack se già presente (evita duplicati)
        ...
        if (!noteOn)
        {
            ...
            {
                // L'envelope è ancora alta durante release: fai retrig veloce
        ...
        for (int i = 0; i < ksmps; ++i)
        {
            switch (envStage)
            {
                ...
                case QUICK_RETRIG:
                    ...
                    envBuf[i] = env; // Lineare per massima velocità
                    break;
...
//             // NON inviamo nulla a Csound, il cambio è gestito da monocpsmidi e noteOnEvent
            
//             else if (!hasNote && hadNote)
//             {
//                 // Tutte le note rilasciate: avvia release
//                 // state.startRelease() è già chiamato in noteOffEvent quando stack diventa vuoto
//                 // Il note-off verrà inviato quando releaseCompleted diventa true
//             }
//         }
//         else
//         {
