...
/**
    ...
    @tags{GUI}
*/
class MyCodeEditorFont   : public Component,public ApplicationCommandTarget,public TextInputTarget
{
public:
    //==============================================================================
    /** Creates an editor for a document.
        ...
        @see CodeDocument
    */
    MyCodeEditorFont (CodeDocument& document,MyCodeTokeniser* codeTokeniser);

    /** Destructor. */
    ~MyCodeEditorFont() override;
    ...
    */
    ...
    struct State
    {
        ...
        void restoreState (MyCodeEditorFont &) const;
    ...
    */
    ...
    struct  MyColourAndFontScheme
    {
        ...
        void set (const String& name, Colour colour,Font myFont);
    };
    ...
    */
    void setColourScheme (const MyColourAndFontScheme& scheme);

    /** Returns the current syntax highlighting colour scheme. */
    const MyColourAndFontScheme& getColourScheme() const noexcept    { return colourScheme; }
    ...
    */
    ...
    Font getFontForTokenType (int tokenType) const;
    /* Restituisce il Font che abbiamo impostato nel Token */
         
    void retokenise (int startIndex, int endIndex);
    
    int pixelToColumn(int line, float x){};
    
    Font getFontUse(int line);
    
    Font getFontUseConst() const;
    ...
    */
    ...
    juce::ScrollBar getScrollBarHorizontal() {return &horizontalScrollBar;}
    juce::ScrollBar getScrollBarVertical() {return &verticalScrollBar;}
...
private:
    ...
    int firstLineOnScreen = 1, spacesPerTab = 4;
    float charWidth = 0;
    int lineHeight = 0, linesOnScreen = 1, columnsOnScreen = 0;
    int scrollbarThickness = 8, columnToTryToMaintain = -1;
    float columnToTryToMaintainPixels = 0.0f;
    ...
    //==============================================================================
    MyCodeTokeniser* codeTokeniser;
    MyColourAndFontScheme colourScheme;
    ...
    int indexToPixelOffset(int lineNum, int indexInLine) const noexcept;
    void scrollToX(int pixelOffset);
    
    juce::String expandTabs(const juce::String& line, int tabSpaces)
    {
        juce::String result;
        int column = 0;
        ...
        for (int i = 0; i < line.length(); ++i)
        {
            ...
            if (c == '\t')
            {
                // Calcola quanti spazi servono per arrivare alla prossima tab stop
                int spaces = tabSpaces - (column % tabSpaces);
                // Aggiungi spazi
                result.append(juce::String::repeatedString(" ", spaces), spaces);
                column += spaces;
            }
            else
            {
    ...
    // float xOffsetPixels = 0.0f; // Sostituto di xOffset * charWidth
    // float yOffsetPixels = 0.0f;
    // float getLineWidthInPixels(int lineIndex) const;


//========================================================================================================================================0

    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MyCodeEditorFont)
};
