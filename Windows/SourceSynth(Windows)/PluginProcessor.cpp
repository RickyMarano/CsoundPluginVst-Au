...
void NewProjectAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    mainInput.setSize(2, samplesPerBlock);
    sidechainInput.setSize(2, samplesPerBlock); 
    midiBuffer.ensureSize(1024);
    midiOutputBuffer.ensureSize(1024);
}
...
#endif
                if(numCsoundChannel == 2 && mainCh == 2)// Input Stereo No Sidechain
                {
                    for (int ch = 0; ch < mainCh; ++ch)
                    {
                        MYFLT InBuffer = mainInput.getReadPointer(ch)[i];
...
// void NewProjectAudioProcessor::StartCsound() STESSO THREAD GRAFICO
// {   
...
//             result = csoundCompileCsdText(csound,editor.documento->getAllContent().toUTF8());
...
void NewProjectAudioProcessor::StartCsound()
{
    ...
    // RegisterMonoMidiOpcodes(csound);
...
{
    if(csound != nullptr && !result && !csoundStopInt)
    {
        result.store(-1);
