...
//Csound API functions for deailing with midi input
    ...
    bool monoMode = true;
    bool retrigger = true;
    Array<int> monoNoteStack;   // stack note premute
    
    int currentNote = -1;
    int lastNote = -1;
    bool noteIsOn = false;
    
    std::atomic<bool> setMono{true};
    std::atomic<bool> setLegato{true};

    //static int ReadMidiDataPoly(CSOUND* csound, void* userData, unsigned char* mbuf, int nbytes);
    //static int ReadMidiDataMono(CSOUND* csound, void* userData, unsigned char* mbuf, int nbytes);
